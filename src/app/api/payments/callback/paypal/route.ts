...
export async function GET(request: Request) {
  ...
  if (!tokenResponse.ok || !token.access_token) return NextResponse.redirect(new URL(`/payment-cancelled?reference=${encodeURIComponent(reference)}`, url.origin));
  const capture = await fetch(`${base}/v2/checkout/orders/${encodeURIComponent(orderId)}/capture`, { method:"POST", headers:{Authorization:`Bearer ${token.access_token}`,"Content-Type":"application/json"}, cache:"no-store" });
  const data = await capture.json();
  const success = capture.ok && data?.status === "COMPLETED";
  await prisma.paymentIntent.update({ where:{id:intent.id}, data:{status:success ? "SUCCESS" : "FAILED", resultCode:success ? 0 : 1, resultDesc:data?.status || null} });
