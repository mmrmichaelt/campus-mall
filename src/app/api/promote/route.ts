...
export async function POST(req: Request) {
  try {
    ...
    const payment = await createPaymentIntent({ userId: user.id, purpose: "PROMOTION", amount: prices[days], phone: String(body.phone || user.phone), paymentMethod: String(body.paymentMethod || "MPESA") as any, metadata: { promotionPurchaseId: purchase.id, listingId: listing.id, days } });
