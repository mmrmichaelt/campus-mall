...
export async function POST(request: Request) {
  try {
    ...
    const intent = await createPaymentIntent({ userId: user.id, purpose: "PRO", amount, phone, metadata: { subscriptionId: subscription.id, proPaymentId: payment.id, plan: parsed.data.plan } });
